...
def get_info_uea(id_interno):
  ...
  if id_interno != 'link':
    ...
    respuesta = '''Nombre de la uea: '''+informacion[0]['nombre']+'''<br>
Clave de la uea: '''+informacion[0]['clave']
    if len(informacion[0]['grupo']) > 0:
      for i in range(len(informacion[0]['grupo'])):
        respuesta += '''<br><br>\n\nGrupo: '''+informacion[0]['grupo'][i]['grupo']+'''\n<br>Profesor:'''+informacion[0]['grupo'][i]['profesor']+'''
\n<br>Horario '''+informacion[0]['grupo'][i]['horario']+'''\nSalón: '''+informacion[0]['grupo'][i]['salon']
      return respuesta
    else:
      return respuesta + '\n<br>El curso no se imparte en el actual trimestre'
  else:
  ...
  if clave != 'datata':
    ...
    try:
      for e in range(len(x)):
        ...
        nombres += informacion[0]['nombre']+'<br>\n'
    except:
      for e in range(len(x)):
...
def get_recomendacion(hist_academico):
  ...
  for elem in ueas_a_recomendar:
    str_recomendacion +=  elem['nombre']+'<br>\n'
...
# iniciar chatbot flask
def trabaja_bot(userText):
  chatbot_name="tutor"
  ai = ChatBot(name=chatbot_name)
  respuesta = preguntar(userText)
  print('Preguntando: ',userText)
  #ai.text_to_speech(respuesta)
  return respuesta
# Execute the AI
